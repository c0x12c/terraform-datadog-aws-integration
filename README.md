...
| <a name="input_namespace_filters_include_only"></a> [namespace\_filters\_include\_only](#input\_namespace\_filters\_include\_only) | Collect metrics only from these AWS CloudWatch namespaces (e.g. `["AWS/ElastiCache", "AWS/RDS"]`). Mutually exclusive with `namespace_filters_exclude_only`. | `list(string)` | All AWS CloudWatch namespaces (see `variables.tf`) | no |
